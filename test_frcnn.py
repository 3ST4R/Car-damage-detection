...
# 5 lists to contain all bounding box coordinates and image names
img_name_list = []
x1_list = []
x2_list = []
y1_list = []
...
def make_color_map():
    ...
    classes_ = ['Scratch_or_spot', 'Dent']
    names = sorted(set(classes_))
    cp = color_palette("Paired", len(classes_))
...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
	...
	for key in bboxes:
		...
		for jk in range(new_boxes.shape[0]):
			...
			#Here's the list of 4 entries that contains the real coordinates
			...
			cv2.rectangle(img,(real_x1, real_y1), (real_x2, real_y2), colormap[key], 2)
			...
			textOrg = (real_x1, real_y1 + baseLine*2)

			#cv2.rectangle(img, (textOrg[0] - 5, textOrg[1]+baseLine - 5), (textOrg[0]+retval[0] + 5, textOrg[1]-retval[1] - 5), (0, 0, 0), 2)
			#cv2.rectangle(img, (textOrg[0] - 5,textOrg[1]+baseLine - 5), (textOrg[0]+retval[0] + 5, textOrg[1]-retval[1] - 5), (255, 255, 255), -1)
			cv2.rectangle(img, pt1, (real_x1 + textSize[0], real_y1 + textSize[1]), colormap[key], thickness=cv2.FILLED)
			cv2.putText(img, textLabel, textOrg, font, scale, (255, 255, 255), thickness)

	print('Elapsed time = {:.3f}'.format(time.time() - st))
	print(all_dets)
	#cv2.imshow('img', img)
	#cv2.waitKey(0)
	cv2.imwrite('./results_imgs/Result {}.png'.format(img_name),img)
