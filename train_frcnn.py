...
parser.add_option("--epoch_len", type="int", dest="epoch_len", help="Number of iterations per epoch.", default=200)
...
try:
	...
	model_rpn.load_weights(C.base_net_weights)
	model_classifier.load_weights(C.base_net_weights)
except:
...
epoch_length = options.epoch_len
	...
	while True:
		try:
			...
			if iter_num == epoch_length:
				...
				if C.verbose:
					print('Mean number of bounding boxes from RPN overlapping ground truth boxes: {:.4f}'.format(mean_overlapping_bboxes))
					print('Classifier accuracy for bounding boxes from RPN: {:.3f}'.format(class_acc))
